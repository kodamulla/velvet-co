...
export default function ViewOrderInfo(props) {
  ...
  return (
    <>
      {/* View Button */}
      <button
        className="bg-primary hover:bg-accent transition-colors px-5 py-2.5 rounded-xl text-white text-sm font-bold shadow-sm shadow-primary/20 cursor-pointer"
      ...
      >
        <div className="p-8 overflow-y-auto custom-scrollbar flex-1">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <select
                    ...
                    className="p-2 rounded-xl border border-secondary text-sm text-text font-semibold bg-background outline-none focus:ring-2 focus:ring-primary/50 cursor-pointer"
                  >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div>
              ...
              <textarea
                className="text-sm text-text font-medium bg-white border border-secondary p-5 rounded-3xl w-full min-h-[100px] outline-none focus:ring-2 focus:ring-primary/50 resize-none"
                value={notes}
          ...
          <div>
            ...
            <div className="space-y-4">
              {order.items.map((item, index) => (
                <div
                  key={index}
                  className="flex flex-col sm:flex-row gap-5 items-center bg-white border border-secondary rounded-3xl p-4 transition-all hover:shadow-md"
                >
                  <img
                    src={item.image}
                    alt={item.name}
        ...
        {/* Footer */}
        <div className="p-6 border-t border-secondary bg-background rounded-b-[32px] flex justify-end gap-4">
          <button
            onClick={() => setIsModalOpen(false)}
          ...
          {(order.notes !== notes || order.status !== status) && (
            <button
              onClick={() => {
                ...
                axios
                  .put(
                    import.meta.env.VITE_BACKEND_URL + "/orders/"+ order._id,
                    {
                      status: status,
                      notes: notes,
                    },
              ...
              className="px-8 py-3 rounded-2xl bg-primary text-white font-bold hover:bg-accent transition-colors shadow-lg shadow-primary/20"
            >
              Save Changes
            </button>
          )}
